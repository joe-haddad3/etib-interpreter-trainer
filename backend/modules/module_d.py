...
def detect_long_silences(segments: list, threshold_seconds: float = 1.2) -> list:
    """Detect clear long pauses, not normal punctuation-length sentence pauses."""
    silences = []
    if not segments:
        return silences
...
def full_evaluation():
    ...
    try:
        ...
        segments_iter, info = model.transcribe(
            ...
            initial_prompt=(
                'Transcribe verbatim. Keep repeated words, repeated phrases, false starts, '
                'hesitations, fillers such as euh, um, uh, and self-corrections exactly as spoken.'
            ),

...
def run_tts(text: str, language: str, accent: str = None,
    ...
    rate_adjustment = max(-50, min(50, int(rate_adjustment or 0)))
    rate_str = f'{rate_adjustment:+d}%' if rate_adjustment != 0 else '+0%'
    filename = f'speech_{uuid.uuid4().hex[:8]}.mp3'
    output_path = os.path.join(AUDIO_OUTPUT_FOLDER, filename)
    # Run TTS in a fresh thread so we never collide with an existing event loop
    # (asyncio.run() raises RuntimeError: "event loop already running" in async hosts).
    import concurrent.futures as _cf
    with _cf.ThreadPoolExecutor(max_workers=1) as ex:
        ex.submit(asyncio.run, _tts_async(text, voice, output_path, rate_str)).result()
...
def generate_materials():
    ...
    try:
        ...
        # Use safe format to prevent crash if speech text contains literal { }
        safe_vals = {k: str(v).replace('{', '{{').replace('}', '}}')
                     for k, v in {'lang_name': lang_names.get(language, 'English'),
                                  'domain': domain, 'script': script}.items()}
        prompt = MATERIALS_PROMPT.format(**safe_vals)

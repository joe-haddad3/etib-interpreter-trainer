...
def detect_repetitions_timing_window(segments: list, language: str = 'ar',
    ...
    NO _SHORT_PARTICLES skip here: a function word appearing twice in the word-
    timestamp list within 1.5 s is always a real disfluency ("mais mais", "le
    le"), not a grammar coincidence.  Text-based detectors handle that filtering.
    """
    words = get_word_timing_sequence(segments)
    repetitions = []
    seen_keys = set()
...
def run_full_analysis(source_script: str, transcript: dict, language: str = 'ar') -> dict:
    ...
    # Word fragments (words cut in half mid-syllable) count as hesitations
